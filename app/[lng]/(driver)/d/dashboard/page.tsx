...
export default async function DriverDashboard({ params }: Props) {
  ...
  return (
    <div className='min-h-screen bg-linear-to-b from-blue-50 to-white p-4 pb-20'>
      ...
      <div className='max-w-md mx-auto mb-6 grid grid-cols-3 gap-3'>
        <div className='bg-white rounded-xl shadow p-4 text-center'>
          ...
          <div className='text-xs text-gray-500 mt-1'>
            {t('driverDashboard.stats.todayRides')}
          </div>
        </div>
        <div className='bg-white rounded-xl shadow p-4 text-center'>
          <div className='text-3xl font-bold text-blue-600'>{rides.length}</div>
          <div className='text-xs text-gray-500 mt-1'>
            {t('driverDashboard.stats.totalRides')}
          </div>
        </div>
        <div className='bg-white rounded-xl shadow p-4 text-center'>
          ...
          <div className='text-xs text-gray-500 mt-1'>
            {t('driverDashboard.stats.vehicle')}
          </div>
        </div>
      </div>

      {/* Navigation Menu */}
      <DriverNavigation hasVehicle={hasVehicle} hasRoute={hasRoute} lng={lng} />
        ...
        <div className='max-w-md mx-auto mb-6'>
          <div className='bg-linear-to-r from-green-500 to-green-600 rounded-2xl shadow-lg p-6 text-white'>
            ...
            <div className='mt-4 text-center text-sm opacity-90'>
              {lng === 'ar'
                ? `${vehicle.route.pricePerSeat} ${t('currency.symbol')} ${t(
                    'driverDashboard.routePrice'
                  )}`
                : `${t('currency.symbol')}${vehicle.route.pricePerSeat} ${t(
                    'driverDashboard.routePrice'
                  )}`}
              {vehicle.route.distance &&
                ` • ${vehicle.route.distance} ${t('driverDashboard.km')}`}
              {vehicle.route.duration &&
                ` • ${vehicle.route.duration} ${t('driverDashboard.min')}`}
            </div>
          </div>
        </div>
      )}
      ...
      {activeRide && (
        <div className='max-w-md mx-auto mb-6'>
          <Link href={`/${lng}/d/dashboard/active-ride`}>
            <div className='bg-green-50 border-2 border-green-300 rounded-2xl p-6 hover:border-green-400 transition-all duration-200 cursor-pointer'>
              <div className='flex items-center gap-3 mb-3'>
                ...
                <h3 className='text-lg font-bold text-green-800'>
                  {t('driverDashboard.activeRideInProgress')}
                </h3>
              </div>
              <p className='text-sm text-green-700'>
                {t('driverDashboard.direction')}:{' '}
        ...
        {canStartRide ? (
          <div className='w-full max-w-md'>
            <h3 className='text-center text-lg font-semibold text-gray-700 mb-4'>
              {t('driverDashboard.chooseDirection')}
            </h3>
          ...
          <div className='w-full max-w-md bg-blue-50 border-2 border-blue-300 rounded-2xl p-6 shadow-lg'>
            <div className='flex flex-col items-center text-center gap-4'>
              ...
              <h3 className='text-xl font-bold text-blue-800'>
                {t('driverDashboard.rideInProgress')}
              </h3>
              <p className='text-base text-blue-700'>
                {t('driverDashboard.completeCurrentRide')}
              </p>
            </div>
          </div>
        ) : (
          <div className='w-full max-w-md bg-yellow-50 border-2 border-yellow-300 rounded-2xl p-6 shadow-lg'>
            <div className='flex flex-col items-center text-center gap-4'>
              ...
              <h3 className='text-xl font-bold text-yellow-800'>
                {t('driverDashboard.setupRequired')}
              </h3>
              <p className='text-base text-yellow-700'>
                {profileCompletion < 100
                  ? t('driverDashboard.completeProfile')
                  : !hasVehicle && !hasRoute
                  ? t('driverDashboard.registerVehicleAndRoute')
                  : !hasVehicle
                  ? t('driverDashboard.registerVehicle')
                  : t('driverDashboard.registerRoute')}
              </p>
            </div>
          </div>
        )}
